...
const slider = useRef(null);
    return(
        <div className={`${styles.bg}`} id="projects">
                <div className={styles.flex} style={{"position":"relative",justifyContent:"center"}}>
                    <div className={`${styles.arrbg} ${styles.left}`} style={{"--dir":"right"}} onClick={() => slider.current.slickPrev()}><Icon path={mdiChevronLeft}size={2}color="white"/></div>
                    ...
                    <div className={`${styles.arrbg} ${styles.right}`} style={{"--dir":"left"}} onClick={() => slider.current.slickNext()}><Icon path={mdiChevronRight}size={2}color="white"/></div>
                </div>
        </div>
    )
}
